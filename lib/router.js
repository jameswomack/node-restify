...
let cors = require('./plugins/cors');
let utils = require('./utils');


///--- Globals

let DEF_CT = 'application/octet-stream';
...
let InvalidVersionError = errors.InvalidVersionError;
let MethodNotAllowedError = errors.MethodNotAllowedError;
let ResourceNotFoundError = errors.ResourceNotFoundError;
let UnsupportedMediaTypeError = errors.UnsupportedMediaTypeError;

let shallowCopy = utils.shallowCopy;
...
function compileURL(options) {
    ...
    let pattern = '^';
    let re;
    let _url = url.parse(options.url).pathname;
...
Router.prototype.render = function render(routeName, params, query) {
    ...
    let route = this.mounts[routeName];

    if (!route) {
        return (null);
    }

    let _path = route.spec.path;
    let _url = _path.replace(/\/:([A-Za-z0-9_]+)(\([^\\]+?\))?/g, pathItem);
    let items = Object.keys(query || {}).map(queryItem);
    let queryString = items.length > 0 ? ('?' + items.join('&')) : '';
...
Router.prototype.unmount = function unmount(name) {
    ...
    let reverse = this.reverse[route.path.source];
    let routes = this.routes[route.method];
...
Router.prototype.find = function find(req, res, callback) {
    ...
    function preflight(methods) {
        ...
        let ok = !headers || headers.split(/\s*,\s*/).every(inAllowedHeaders);
    ...
    for (j = 0; j < urls.length; j++) {
        if (matchURL(new RegExp(urls[j]), req)) {
            ...
            let err = new MethodNotAllowedError('%s is not allowed',
                req.method);
            callback(err);
            return;
        }
...
Router.prototype.toString = function toString() {
    ...
    Object.keys(this.routes).forEach(function (k) {
        let routes = self.routes[k].map(function (r) {
            return (r.name);
        });
