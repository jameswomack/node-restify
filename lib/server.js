...
function Server(options) {
    ...
    const fmt = mergeFormatters(options.formatters);
    ...
    if (options.handleUpgrades) {
        this.server.on('upgrade', (req, socket, head) => {
            ...
            const res = upgrade.createResponse(req, socket, head);
            self._setupRequest(req, res);
    ...
    this.__defineGetter__('maxHeadersCount', () => {
        return (self.server.maxHeadersCount);
    });

    this.__defineSetter__('maxHeadersCount', (c) => {
...
[
    ...
    'patch'
        ...
        Server.prototype[method] = function (...args) {
            ...
            argumentsToChain(args, 1).forEach(addHandler);
...
Server.prototype.rm = function rm(route) {
    const r = this.router.unmount(route);
...
Server.prototype.use = function use(...args) {
    (argumentsToChain(args) || []).forEach((h) => {
        this.chain.push(h);
    });
 ...
 * @returns {Object} returns self
 */
Server.prototype.pre = function pre(...args) {
    argumentsToChain(args).forEach((h) => {
        this.before.push(h);
    });
...
Server.prototype.toString = function toString() {
    ...
    Object.keys(this.routes).forEach((k) => {
        const handlers = handlersToString(this.routes[k]);
...
Server.prototype._handle = function _handle(req, res) {
    ...
    function routeAndRun() {
        self._route(req, res, (route, context) => {
            ...
            self._run(req, res, route, chain, (e) => {
    ...
    if (this.before.length > 0) {
        this._run(req, res, null, this.before, (err) => {
            if (!err) {
                routeAndRun();
            }
        });
...
Server.prototype._route = function _route(req, res, name, cb) {
    ...
    if (typeof (name) === 'function') {
        ...
        this.router.get(name, req, (err, route, ctx) => {
            if (err) {
                emitRouteError(self, req, res, err);
            } else {
                cb(route, ctx);
...
Server.prototype._run = function _run(req, res, route, chain, cb) {
    ...
    function next(arg) {
        ...
        if (arg) {
            if (arg instanceof Error) {
                ...
                if (self.listeners(errName).length > 0) {
                    self.emit(errName, req, res, arg, once(() => {
                        res.send(arg);
                ...
                if (req._rstfy_chained_route) {
                    const _e = new errors.InternalError();
                    log.error({
                        err: _e
                ...
                return self._route(req, res, arg, (r, ctx) => {
                    req.context = req.params = ctx;
                    req.route = r.spec;

                    const _c = chain.slice(0, i + 1);

                    function _uniq(fn) {
                        return (_c.indexOf(fn) === -1);
                    }

                    const _routes = self.routes[r.name] || [];
                    const _chain = _routes.filter(_uniq);
        ...
        dtrace._rstfy_probes['route-done'].fire(() => {
            return ([
                self.name,
    ...
    dtrace._rstfy_probes['route-start'].fire(() => {
        return ([
            self.name,
            route !== null ? route.name : 'pre',
            id,
