...
function Server(options) {
    ...
    let fmt = mergeFormatters(options.formatters);
    ...
    if (options.handleUpgrades) {
        this.server.on('upgrade', function onUpgrade(req, socket, head) {
            ...
            let res = upgrade.createResponse(req, socket, head);
            self._setupRequest(req, res);
...
Server.prototype.rm = function rm(route) {
    let r = this.router.unmount(route);
...
Server.prototype.toString = function toString() {
    let LINE_FMT = '\t%s: %s\n';
    let SUB_LINE_FMT = '\t\t%s: %s\n';
    let self = this;
    let str = '';

    function handlersToString(arr) {
        let s = '[' + arr.map(function (b) {
    ...
    Object.keys(this.routes).forEach(function (k) {
        let handlers = handlersToString(self.routes[k]);
        ...
        if (arg) {
            if (arg instanceof Error) {
                ...
                if (req._rstfy_chained_route) {
                    let _e = new errors.InternalError();
                    log.error({
                        err: _e
                ...
                return self._route(req, res, arg, function (r, ctx) {
                    ...
                    let _c = chain.slice(0, i + 1);

                    function _uniq(fn) {
                        return (_c.indexOf(fn) === -1);
                    }

                    let _routes = self.routes[r.name] || [];
                    let _chain = _routes.filter(_uniq);
