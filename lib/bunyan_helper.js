...
let DEFAULT_REQ_ID = uuid.v4();
let STR_FMT = '[object %s<level=%d, limit=%d, maxRequestIds=%d>]';
...
RequestCaptureStream.prototype.write = function write(record) {
    let req_id = record.req_id || DEFAULT_REQ_ID;
    let ring;
    let self = this;
    ...
    if (record.level >= this.level) {
        ...
        if (this.dumpDefault) {
            let defaultRing = self.requestMap.get(DEFAULT_REQ_ID);
